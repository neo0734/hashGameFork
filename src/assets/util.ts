...
import type { DevInspectResults } from '@onelabs/sui/client';
...
export const addPoint = (address: string, len = 5) => {
  return address
    ? address.substr(0, len) + '...' + address.substr(address.length - len)
    : '';
};
...
export function getCoinOutWithFees(
  ...
  const reserveInSize_after = new BigNumber(reserveIn)
    .plus(amountInAfterFee)
    .toNumber();
  const reserveOutSize_after = new BigNumber(k)
    .dividedBy(reserveInSize_after)
    .toNumber(); //bignumber
  // x*y=k: out = (amountInAfterFee * reserveOut) / (reserveIn + amountInAfterFee)
  const out = new BigNumber(reserveOut)
    .minus(reserveOutSize_after)
    .multipliedBy(1 - slippageVal)
    .toNumber();
  return out > 0 ? out : 0;
}
export function getCoinInWithFees(
  ...
  const reserveOutSize_after = new BigNumber(k)
    ...
    .toNumber(); //bignumber
  // x*y=k: out = (amountInAfterFee * reserveOut) / (reserveIn + amountInAfterFee)
  const out = new BigNumber(reserveOutSize_after)
    .minus(reserveOut)
    .multipliedBy(1 + slippageVal)
